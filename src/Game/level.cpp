...
std::shared_ptr<IGameObject> createGameObjectFromDescription(const char descriprion, const glm::vec2& position, const glm::vec2& size, const float rotation)
{
	switch (descriprion)
	{
	case '0':
	...
	case 'D':
		return nullptr;
	default:
		std::cerr << "Unknown GameObject description :" << descriprion << std::endl;
		break;
	}
		return nullptr;
}
